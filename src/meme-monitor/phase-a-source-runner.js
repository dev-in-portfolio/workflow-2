...
async function fetchAlpacaMarketSignals({ env, fetchImpl, symbols = [], timeoutMs = 5000, repoRoot = process.cwd() } = {}) {
  ...
  try {
    ...
    for (const symbol of symbols) {
      ...
      const previousClose = safeNumber(snapshot.previousDailyBar?.c ?? snapshot.previous_daily_bar?.c ?? snapshot.dailyBar?.o ?? snapshot.daily_bar?.o, null);
      const volume = safeNumber(snapshot.dailyBar?.v ?? snapshot.daily_bar?.v ?? latestTrade?.s, null);
      const averageVolume = safeNumber(snapshot.previousDailyBar?.v ?? snapshot.previous_daily_bar?.v ?? null, null);

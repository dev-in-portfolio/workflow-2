...
const REGULAR_STOCK_START_MINUTES = 5 * 60;
const REGULAR_STOCK_STOP_MINUTES = 17 * 60;
...
function resolveRegularStockAutomationSchedule(now = new Date()) {
  ...
  const nextStartDay = resolveNextMarketDay(current, {
    includeToday: marketDay && currentMinutes !== null && currentMinutes < REGULAR_STOCK_START_MINUTES,
  });
  const nextStopDay = resolveNextMarketDay(current, {
    includeToday: marketDay && currentMinutes !== null && currentMinutes < REGULAR_STOCK_STOP_MINUTES,
  });

  const start = buildScheduleMoment(nextStartDay, REGULAR_STOCK_START_MINUTES, now);
  const stop = buildScheduleMoment(nextStopDay, REGULAR_STOCK_STOP_MINUTES, now);
...
function buildScheduleNote({ current, marketDay, start, stop }) {
  ...
  if (currentMinutes !== null && currentMinutes < REGULAR_STOCK_START_MINUTES) {
    return `Market opens later today; start is queued for ${start.label} and stop for ${stop.label}.`;
  }
  if (currentMinutes !== null && currentMinutes < REGULAR_STOCK_STOP_MINUTES) {
...
module.exports = {
  NEW_YORK_TIME_ZONE,
  REGULAR_STOCK_START_MINUTES,
  REGULAR_STOCK_STOP_MINUTES,
  addDays,

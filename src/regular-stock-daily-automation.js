...
} = require('./regular-stock-schedule');
...
async function runRegularStockDailyAutomation(options = {}) {
  ...
  const schedule = resolveRegularStockAutomationSchedule(now);
  ...
  const marketState = {
    timestamp: nowIso(),
    weekday: getNewYorkCalendarParts(now).weekday,
    holiday: isUsMarketHoliday(now),
    market_day: isUsMarketWeekday(now) && !isUsMarketHoliday(now),
    schedule,
  };
...
function buildWarnings({ action, control, snapshot, actionResult, marketState }) {
  ...
  if (action === 'start' && marketState.market_day === false) {
...
function formatAutomationSummary(summary) {
  ...
  lines.push(`Schedule: ${summary.market_state?.schedule?.start?.label || '-'} -> ${summary.market_state?.schedule?.stop?.label || '-'}`);
...
module.exports = {
  ...
  runRegularStockDailyAutomation,
};

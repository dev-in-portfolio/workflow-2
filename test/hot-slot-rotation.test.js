...
test('hot slot rotation defaults to a shorter post-exit recheck window', () => {
  const config = resolveHotSlotRotationConfig({
    MEME_HOT_SLOT_ROTATION_ENABLED: 'true',
  });

  assert.equal(config.exitTimeoutSeconds, 8);
  assert.equal(config.entryRecheckMaxAgeSeconds, 45);
});
...
test('scanner rotation blocks a candidate that goes stale after the exit', async () => {
  ...
  try {
    ...
    assert.equal(result.accepted, true);
    const runtime = JSON.parse(fs.readFileSync(path.join(dataDir, 'state', 'scanner-runtime.json'), 'utf8'));
    assert.notEqual(runtime.hot_slot_rotation.lastDecision, 'rotation_reconcile_after_exit_failed');
  } finally {
    scanner.stop();
    fs.rmSync(tempRoot, { recursive: true, force: true });
  }
...
test('scanner rotation falls back to the freshest current candidate after the exit', async () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'hot-slot-rotation-fallback-'));
  const dataDir = path.join(tempRoot, 'data');
  fs.mkdirSync(path.join(dataDir, 'state'), { recursive: true });
  fs.mkdirSync(path.join(dataDir, 'runtime'), { recursive: true });
  const env = {
  ...
  try {
    const result = await scanner.runOnce({ runId: 'rotation-fallback' });
    assert.equal(result.accepted, true);
    assert.equal(requests[0].side, 'sell');
    assert.equal(requests[1].symbol, 'AAA');
    assert.equal(requests[1].side, 'buy');
    assert.equal(requests.some((entry) => entry.symbol === 'SOUN' && entry.side === 'buy'), false);
    const runtime = JSON.parse(fs.readFileSync(path.join(dataDir, 'state', 'scanner-runtime.json'), 'utf8'));
    assert.equal(runtime.hot_slot_rotation.lastDecision, 'rotation_complete');
    assert.equal(runtime.hot_slot_rotation.candidate, 'AAA');
  } finally {
    scanner.stop();
    fs.rmSync(tempRoot, { recursive: true, force: true });
  }
